...
/* ファイルを開く ダイアログ */
static const char *openGetFile(const char* title)
{
	...
	if ([panel runModal] == NSFileHandlingPanelOKButton) {
		NSURL *aURL = [panel URL];
		return [aURL fileSystemRepresentation];
	}
...
/* ファイルを保存ダイアログ */
static const char *openPutFile(const char* title)
{
...
static	void	f_drv_chg(int data)
{
    ...
    if ((filename = openGetFile(headline))) {
...
static	void	f_misc_csave_s(void)
{
    ...
    if ((filename = openPutFile(headline))) {
