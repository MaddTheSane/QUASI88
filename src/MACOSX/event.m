...
void	event_update(void)
{
	...
	while (loop) {
	    ...
	    case inDrag:
						if (quasi88_is_exec()) xmame_sound_suspend();
					{
#warning replace macQd.screenBits.bounds!
...
void	event_get_mouse_pos(int *x, int *y)
{
	...
	curPoint = [[appDel mainWindow] mouseLocationOutsideOfEventStream];
	curPoint = [[appDel.mainWindow contentView] convertPoint:curPoint fromView:nil];
	*x = curPoint.x < 0 ? 0 : curPoint.x;
    *y = curPoint.y < 0 ? 0 : curPoint.y;
}
